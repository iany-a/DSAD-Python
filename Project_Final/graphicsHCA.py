...
def dendrogram(h, labels=None, title='Hierarchical classification',
    ...
    f = plt.figure(figsize=(12, 8))  # Increased height slightly for labels
    ax = f.add_subplot(1, 1, 1)
    ax.set_title(title, fontsize=14, color='k')

    if colors is None:
        hclust.dendrogram(h, labels=labels, leaf_rotation=45,  # 45 degrees often looks cleaner
                          leaf_font_size=10, ax=ax,
                          color_threshold=threshold)
    else:
        hclust.dendrogram(h, labels=labels, leaf_rotation=45, ax=ax,
                          leaf_font_size=10,
                          link_color_func=lambda k: colors[k])

    # --- THE FIX FOR ALIGNMENT ---
    # This grabs the labels and anchors the top-right of the text to the plot
    plt.setp(ax.get_xticklabels(), rotation=45, ha='right', rotation_mode="anchor")

    if threshold is not None:
        plt.axhline(y=threshold, color='r', linestyle='--')

    # Ensures the labels fit within the figure area
    plt.tight_layout()
